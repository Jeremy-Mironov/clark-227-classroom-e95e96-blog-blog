...
if (!$author_id) {
    ...
    if (!$author) {
        ...
        if($page > $total_pages && $total_pages > 0){
            header("Location: author.php?id={$author_id}&page=" . $total_pages . ($sort !== 'newest' ? "&sort=" . $sort : ''));
            exit;
        }
        ...
        if (count($data) === 0) {
            ...
            if($total_pages > 1){
                ...
                if($page > 1){
                    $prev_link = "author.php?id={$author_id}&page=" . ($page - 1) . ($sort !== 'newest' ? "&sort=" . $sort : '');
                ...
                for($i = 1; $i <= $total_pages; $i++){
                    if($i === $page){
                        ...
                        $page_link = "author.php?id={$author_id}&page={$i}" . ($sort !== 'newest' ? "&sort=" . $sort : '');
                        echo "<li class='page-item'><a class='page-link' href='{$page_link}'>{$i}</a></li>";
                    }
                }
                
                if($page < $total_pages){
                    $next_link = "author.php?id={$author_id}&page=" . ($page + 1) . ($sort !== 'newest' ? "&sort=" . $sort : '');
